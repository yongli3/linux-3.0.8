...
void s3c_fimc0_cfg_gpio(struct platform_device *pdev)
{
	...
	for (i = 0; i < 8; i++) {
		...
		s5p_gpio_set_drvstr(S5PV210_GPE0(i), S5P_GPIO_DRVSTR_LV4);
	}
	/* CAM A port(b0010) : DATA[5-7], CLKOUT(MIPI CAM also), FIELD */
	for (i = 0; i < 5; i++) {
		s3c_gpio_cfgpin(S5PV210_GPE1(i), S3C_GPIO_SFN(2));
		s3c_gpio_setpull(S5PV210_GPE1(i), S3C_GPIO_PULL_UP);
		s5p_gpio_set_drvstr(S5PV210_GPE1(i), S5P_GPIO_DRVSTR_LV4);
	}
	...
	for (i = 0; i < 5; i++) {
		...
		s5p_gpio_set_drvstr(S5PV210_GPJ1(i), S5P_GPIO_DRVSTR_LV4);
	}
#endif
