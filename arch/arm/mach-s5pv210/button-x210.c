...
static int s3c_button_probe(struct platform_device *pdev)
{
	...
	/* gph2_1 (right) */
	ret = gpio_request(S5PV210_GPH2(1), "GPH2");
	if(ret)
		printk("button-smdkv210: request gpio GPH2(1) fail");

	/* gph2_3 (menu) */
	ret = gpio_request(S5PV210_GPH2(3), "GPH2");
	if(ret)
		printk("button-smdkv210: request gpio GPH2(3) fail");

	/* gph2_2 (back) */
	ret = gpio_request(S5PV210_GPH2(2), "GPH2");
	if(ret)
		printk("button-smdkv210: request gpio GPH2(2) fail");
	...
	s3c_button_history[3] = gpio_get_value(S5PV210_GPH2(0));
	s3c_button_history[4] = gpio_get_value(S5PV210_GPH2(1));
	s3c_button_history[5] = gpio_get_value(S5PV210_GPH2(2));
	s3c_button_history[6] = gpio_get_value(S5PV210_GPH2(3));
