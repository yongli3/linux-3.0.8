...
extern int s3c_adc_get_adc_data(void);
...
static int s3c_adc_get_data_with_filter(void)
{
	int adc_data;
	int adc_max = 0;
	int adc_min = 0;
...
{	
	...
	value = s3c_adc_get_data_with_filter();
	if(value <= 0)
		return;
...
static void s3c_adc_bat_work(struct work_struct *work)
{
	...
	schedule_delayed_work(&bat_work, msecs_to_jiffies(1000));
}
