...
static u16 my_reg[WM8978_CACHEREGNUM]={0,29,405,367,16,0,0,0,0,0,0,
...
static struct snd_soc_codec * gcodec = 0;
void wm8978_headset(int on)
{
	if(!gcodec)
		return;
	...
	if(on)
	{
		my_reg[0x34] = 0x40 | 0x0100;
		my_reg[0x35] = 0x40 | 0x0100;
		my_reg[0x36] = 0x3d | 0x0100;
		my_reg[0x37] = 0x3d | 0x0100;
		my_reg[0x38] = 0x04;
		my_reg[0x39] = 0x5f;

		snd_soc_write(gcodec, 0x34, 0x40 | 0x0100);
		snd_soc_write(gcodec, 0x35, 0x40 | 0x0100);
		snd_soc_write(gcodec, 0x36, 0x3d | 0x0100);
		snd_soc_write(gcodec, 0x37, 0x3d | 0x0100);
		snd_soc_write(gcodec, 0x38, 0x04);
		snd_soc_write(gcodec, 0x39, 0x5f);

		printk("headphone plug out\n");
	}
	else
	{
		my_reg[0x34] = 0x1f | 0x0100;
		my_reg[0x35] = 0x1f | 0x0100;
		my_reg[0x36] = 0x40 | 0x0100;
		my_reg[0x37] = 0x40 | 0x0100;
		my_reg[0x38] = 0x04;
		my_reg[0x39] = 0x5f;

		snd_soc_write(gcodec, 0x34, 0x1f | 0x0100);
		snd_soc_write(gcodec, 0x35, 0x1f | 0x0100);
		snd_soc_write(gcodec, 0x36, 0x40 | 0x0100);
		snd_soc_write(gcodec, 0x37, 0x40 | 0x0100);
		snd_soc_write(gcodec, 0x38, 0x04);
		snd_soc_write(gcodec, 0x39, 0x5f);
